...
do
    ...
    if [ -f $file ]; then
        fgrep -f ../../data/sirvs_mappings/HS.erccSpikein.ids.list $file | while read text cap sf labx chr strt stp ID count
        do
            echo -e "$sample\t$platform\t$ID\t$count\tERCC\t$lab\t$cap"
        done

        fgrep -f ../../data/sirvs_mappings/HS.notTargeted.erccSpikein.ids.list $file | while read text cap sf labx chr strt stp ID count
        do
            echo -e "$sample\t$platform\t$ID\t$count\tERCC\t$lab\t$cap"
        done
    fi
...
do
    cat /users/rg/buszczynska/Projects/RNA.capture/phase3/spikein.concentr/ERCC.concs | awk -v spike=$ERCC '$2==spike'| while read nb spikeIn subgroup conc1 conc2 FC sth
    do
