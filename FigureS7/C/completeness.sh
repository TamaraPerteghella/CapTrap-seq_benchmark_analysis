...
do
    ...
    if [[ $cap == "MpreCap" ]]; then
        cage="../../data/mouse_CAGE_peaks.bed"
    else
        cage="../../data/human_CAGE_peaks.bed"
    fi

    zcat $original | egrep -v "SIRVome_isoforms|ERCC" | awk -F"\t" -v OFS="\t" '{ if( $6 == "+" ) print $1, $2-50, $2+50, $4, "0", $6; else print $1, $3-50, $3+50, $4, "0", $6 }' | awk -F"\t" -v OFS="\t" '{ if( $2 < 0 ) print $1, "0", $3, $4, $5, $6; else print $0 }' > ${lab}_${cap}_${tmp}_${sample}.starts.bed
       
    bedtools intersect -u -s -a ${lab}_${cap}_${tmp}_${sample}.starts.bed -b $cage > ${lab}_${cap}_${tmp}_${sample}.crossCAGE.bed
    Rscript ../../Figure1/E/completeness_collate.R $original ${lab}_${cap}_${tmp}_${sample}.crossCAGE.bed ${lab}_${cap}_${tmp}_${sample}.polyAreads.refined.list ${lab}_${cap}_${tmp}_${sample}.sirvs.bed
done < ../../Figure5/samples.lrgasp.tsv 

while read lab cap tmp sample
do
    cat ${lab}_${cap}_${tmp}_${sample}.count.tsv
done < ../../Figure5/samples.lrgasp.tsv  > lrgasp.support.stats.tsv
